...
export const gameRegistry: Record<MvpGameType, GameDefinition> = {
  ...
  charades: {
    ...
    guide: {
      ...
      steps: [
        ...
        "설명자만 화면의 제시어를 확인합니다.",
        "설명자는 말이나 글자 표현 없이 몸으로 제시어를 설명합니다.",
  ...
  three_in_time: {
    ...
    guide: {
      ...
      steps: [
        ...
        "타이머가 시작되면 팀은 조건에 맞는 답을 연속으로 말합니다.",
  ...
  progressive_hint: {
    ...
    guide: {
      ...
      steps: [
        "첫 번째 힌트만 공개하고 도전을 받습니다.",
        "정답이 나오지 않으면 다음 힌트를 공개합니다.",
        "오답 정책에 따라 상대팀 기회 또는 재도전을 진행합니다.",
  ...
  football_career: {
    ...
    guide: {
      objective: "선수의 소속 클럽 경력을 단계별로 보고 어떤 선수인지 맞히는 축구 퀴즈입니다.",
      steps: [
        "첫 단계의 소속 클럽을 공개합니다.",
        "정답이 없으면 다음 단계에서 클럽 경력을 더 공개합니다.",
        "팀은 선수 이름을 말하고 사회자가 정답을 판정합니다.",
        "정답 또는 최종 단계 공개 후 다음 선수로 이동합니다.",
      ],
      scoring: ["적은 클럽만 보고 맞힐수록 높은 단계 점수를 받습니다.", "인정 가능한 이름과 별명은 정답 데이터에 포함됩니다."],
